...
**libmultistreaming** is a forked repository from [libstreaming](https://github.com/fyhertz/libstreaming), offering some new functionalities and an easier use. The changes have been made as part of a series of Final Projects for the Computer Engineering degree at the Universidad Complutense de Madrid(UCM). The changes have been made by different students under the tutoring of Simon Pickin, a professor at this university, and all of them have given consent for this library to be published under an Apache License. The projects under which the code has been developed are the following:
  - ''Device to Device streaming en dispositivos móviles'', by Iván Gulyk (@ivangulyk) and Noel José Algora Igual (@NoelAlgora)
  - ''Multistreaming of video and audio in an ad-hoc network of common mobile devices'' by Daniel Alfaro Miranda and Luis Pozas Palomo (@luispozas)
  - ''Transmission of the stream generated by the camera and microphone of a mobile device through an ad-hoc network of mobile devices.'' by Jingru Zhao (@zhaojr0514)

Another project has also worked on this development, although for technical and bureaucratic reasons they couldn't advance on the code, just contribute to the investigation:
  - ''Crowdstreaming: video and audio transmission in a MANET (mobile ad hoc network)'' by Francisco Calero Velasco (@FCalero), Sergio Manzanaro Caraballo (@SergioManzanaro), and David Salido Camacho (@davidsalido)

Finally, under this last project, all code has been detached from the application where it was developed and changed to adapt it to be an Android module:
  - ''Detaching and publishing changes in the libstreaming library for the CrowdStreaming application'' by Miguel Verdaguer (@verdx)
